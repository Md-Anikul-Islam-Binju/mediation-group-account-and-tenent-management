...
@section('admin_content')
                ...
                <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                    ...
                    @foreach($tenants as $key => $tenant)
                        <tr>
                            ...
                            <td>
                                @if(!empty($tenant->agreement_paper) && is_array($tenant->agreement_paper))
                                    @foreach($tenant->agreement_paper as $key => $file)
                                        @php
                                        ...
                                        <div style="margin-left: {{ $indent }}px;" class="mb-1">
                                            <a href="{{ asset('images/tenant_agreements/' . $file) }}"
                                               target="_blank"
                                               class="badge bg-info">
                                                {{ $label }}
                                            </a>
                            ...
                            <td>
                                <span>{{ $tenant->account_mode ?? 'N/A' }}</span>
                            </td>



                            <td>{{ $tenant->status == 'active' ? 'Active' : 'Inactive' }}</td>
                                ...
                                <div class="d-flex justify-content-end gap-1">
                                    @can('owner-edit')
                                        <button class="btn btn-info btn-sm"
                                                ...
                                                data-bs-target="#editNewModalId{{ $tenant->id }}">
                                            Edit
                                        </button>
                                    @endcan

                                    @can('owner-delete')
                                        <a href="{{ route('owner.destroy', $tenant->id) }}"
                                           class="btn btn-danger btn-sm"
                                           data-bs-toggle="modal"
                                           data-bs-target="#danger-header-modal{{ $tenant->id }}">
                                            Delete
                                        </a>
                                    @endcan
                                </div>
                        ...
                        {{-- Edit Modal --}}
                        <div class="modal fade" id="editNewModalId{{ $tenant->id }}" data-bs-backdrop="static" tabindex="-1">
                                        ...
                                        <form method="POST" action="{{ route('tenant.update', $tenant->id) }}" enctype="multipart/form-data">
                                            ...
                                            <div class="row">
                                                <div class="col-12 mb-3">
                                                    ...
                                                    <input type="text" name="organization" value="{{ $tenant->organization }}" class="form-control">
                                                </div>
                                            </div>
                                            ...
                                            <div class="row">
                                                <div class="col-6 mb-3">
                                                    ...
                                                    <input type="file"
                                                           id="editAgreementFiles{{ $tenant->id }}"
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            onclick="document.getElementById('editAgreementFiles{{ $tenant->id }}').click()">
                                                        <i class="mdi mdi-upload"></i> Add Files
                                                    </button>

                                                    <div id="editFilePreview{{ $tenant->id }}" class="mt-2"></div>
                                                ...
                                                <div class="col-6 mb-3">
                                                    ...
                                                    <select name="account_mode" class="form-select">
                                                        <option value="Cash" {{ $tenant->account_mode == 'Cash' ? 'selected' : '' }}>Cash</option>
                                                        <option value="BEFTIN" {{ $tenant->account_mode == 'BEFTIN' ? 'selected' : '' }}>BEFTIN </option>
                                                        <option value="RTGS" {{ $tenant->account_mode == 'RTGS' ? 'selected' : '' }}>RTGS</option>
                                                        <option value="ATM" {{ $tenant->account_mode == 'ATM' ? 'selected' : '' }}>ATM</option>
                                                    </select>
                                                </div>
                        ...
                        {{-- Delete Modal --}}
                        <div id="danger-header-modal{{ $tenant->id }}" class="modal fade" tabindex="-1">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{ route('tenant.destroy', $tenant->id) }}" class="btn btn-danger">Delete</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('tenant.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6 mb-3">
                                <label>Account Mode</label>
                                <select name="account_mode" class="form-select" required>
                                    <option value="">Select Mode</option>
                                    <option value="Cash">Cash</option>
                                    <option value="BEFTIN">BEFTIN</option>
    ...
    @push('scripts')
        <script>
            ...
            setupFileUpload(
                'editAgreementFiles{{ $tenant->id }}',
                'editFilePreview{{ $tenant->id }}'
            );
            @endforeach
